...
`tmp/plans/.claimed/01-traces-model-filter-broken.md`

## Problem

The model filter on the traces page returns no results because model information
is not reliably stored in the traces table. The work item status is "Review Required"
meaning a previous implementation exists but needs validation and potential fixes.
...
LangSmith SDK  ->  POST /runs/multipart  ->  runs_multipart()  ->  proxy.record_run()
                   POST /runs             ->  runs_create()          |
                                                          propagate_model_to_root()
                                                                     |
                                                          SQLite traces.model column
```
...
Dedicated `model TEXT NOT NULL DEFAULT ''` column on the traces table.
...
Since this is a "Review Required" item, the implementation task should:
1. Verify the existing code correctly populates the model column
2. Verify the filter works end-to-end with the model column
3. Verify the model column displays in the UI
