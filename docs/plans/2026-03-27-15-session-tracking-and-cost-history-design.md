...
This item was previously designed (2026-03-26) but not implemented. The DB schema,
lifecycle, and frontend changes described below need to be built from scratch.
...
New proxy helpers:
- `create_session(label=None) -> int` -- INSERT, return row id; close any orphaned
  sessions (end_time IS NULL) first for crash-recovery
