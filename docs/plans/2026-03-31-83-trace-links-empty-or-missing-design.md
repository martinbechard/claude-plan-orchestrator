...
Requirements: docs/plans/2026-03-31-83-trace-links-empty-or-missing-requirements.md
...
| AC3 | D2, D4 | Synthetic trace row carries top-level info; JS renders step name, status, duration |
| AC4 | D1, D3 | UUID always written to item file; supervisor always reads it; record_completion always stores it |
| AC5 | D1, D2 | UUID generated from same code path as before (valid format); synthetic trace row references it |
| AC6 | D4 | JS fetches tree from API and renders in the page shell; link already navigates correctly |
| AC7 | D4 | JS renders nested collapsible tree showing parent/child hierarchy |
| AC8 | D4 | Tree nodes show step name, status, duration, nesting depth via indentation/collapse |
