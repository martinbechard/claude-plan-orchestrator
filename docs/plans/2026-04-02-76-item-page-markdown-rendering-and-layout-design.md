...
Requirements: docs/plans/2026-04-02-76-item-page-markdown-rendering-and-layout-requirements.md
Date: 2026-04-02
...
- Approach: Add the marked.js library (via CDN with SRI hash) to item.html. In
  ...
  markdown elements rendered consistently).
  ...
  AC3 is satisfied because the shared helper is called from both code paths.
  AC4 is satisfied because marked.js supports all standard GFM elements and
  the shared helper ensures consistent application across all artifact types.
- Files: langgraph_pipeline/web/templates/item.html

### D2: CSS layout constraints for two-column stability

- Addresses: P2, FR2
- Satisfies: AC5, AC6, AC7, AC8, AC9
- Approach: Apply three CSS fixes:
  ...
  AC5 is satisfied because expanding an artifact section can no longer push or
  overlap the right column -- min-width: 0 constrains the grid track.
  AC6 is satisfied because the right column stays fully visible and positioned.
  AC7 is satisfied because content containers are constrained to column width
  via min-width: 0 + max-width: 100%.
  AC8 is satisfied because CSS constraints are static rules, independent of
  expand/collapse toggle state.
  AC9 is satisfied because wide content (e.g. code blocks, tables) gets
...
| AC2: Headings, lists, code blocks correct | D1, D3 | marked.js converts markdown constructs to HTML elements; D3 styles with scaled sizes, indentation, monospace |
| AC3: Every artifact section renders markdown as HTML | D1, D3 | Shared renderArtifactContent() helper called from both loadStageArtifacts() and artifact-link handler |
| AC4: Markdown rendering consistent across all artifact types | D1, D3 | Same shared helper and CSS rules applied uniformly to all artifact sections |
| AC5: Expanding avoids pushing/overlapping right column | D2 | min-width: 0 on grid children prevents left column from exceeding its track |
| AC6: Right column visible and positioned when expanded | D2 | CSS grid constraints keep right column at fixed 360px regardless of left content |
| AC7: Expanded content within column boundaries | D2 | Content containers constrained to column width via min-width: 0 + max-width: 100% |
| AC8: Sidebar unaffected by expand/collapse | D2 | CSS constraints are static rules, independent of toggle state |
| AC9: Overflow handled gracefully | D2 | overflow-x: auto provides horizontal scrolling for wide content |
