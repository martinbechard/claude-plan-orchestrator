...
Three agents join the existing coder and code-reviewer:
...
Expand infer_agent_for_task() to recognize design-related keywords:
...
1. List all five agents (coder, code-reviewer, systems-designer, ux-designer, planner)
...
For features requiring both architectural and UX design, the Phase 0 template supports dispatching complementary agent pairs. This is not a code feature but a plan structure pattern:
...
The judge task (0.6) then evaluates both the systems and UX designs as complementary pairs, scoring each pair holistically.
...
| File | Change |
|------|--------|
| scripts/plan-orchestrator.py | Add DESIGNER_KEYWORDS, PLANNER_KEYWORDS; expand infer_agent_for_task() |
| scripts/auto-pipeline.py | Update PLAN_CREATION_PROMPT_TEMPLATE agent list |
| .claude/skills/implement/SKILL.md | Update Phase 0 template with agent assignments |
...
4. **Agent teams are a plan pattern, not orchestrator code.** The existing parallel_group mechanism handles agent teams naturally. No new orchestrator code is needed for team dispatch - it is achieved by placing different-agent tasks in the same parallel group.
